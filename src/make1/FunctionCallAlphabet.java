...
public class FunctionCallAlphabet
{
	public static void main( String[] args )
	{
		...
		i(32);// ???? i( ???? );    // pass it any integer and it will display a word beginning with I 
		j(1);// ???? j( ???? );    // returns a different word depending on what you pass it (1-3)
		k();// ???? k( ???? );    // displays a word beginning with K
		boolean a = true; boolean b = false; l(a, b); // ???? l( ???? );    // displays a different word depending on the two booleans you pass it
		m(a,b);// ???? m( ???? );    // displays a different word depending on the two booleans you pass it
		...
		w(2);// ???? w( ???? );    // pass it any integer and it will display a word beginning with W 
		x(2);// ???? x( ???? );    // returns a different word depending on what you pass it (1-2)
		y(); // ???? y( ???? );    // displays a word beginning with Y
		z(a, b); // ???? z( ???? );    // returns a different word depending on which two boolean values you pass it
		System.out.println();
	}
